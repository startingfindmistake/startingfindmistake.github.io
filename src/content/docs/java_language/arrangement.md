...
```
* 모든 배열은 `0`부터 시작 합니다.
길이가 `n`인 배열은 `0`부터 `n-1`까지의 정수로 인덱싱 될수 있습니다.
* 각 요소는 숫자 인덱스로 접근합니다.
번호 매기기는 `0`부터 시작합니다.
따라서 `9`번째 요소는 인덱스 `8`로 접근해야 합니다.

>즉, `int[3]`으로 선언하면 길이는 3이지만,
> 규칙에 따라 인덱스는 `0`부터 `3 - 1`까지인 `2`까지만 사용해야 한다고 문법적 규칙으로 못 박아 두고 있습니다.
